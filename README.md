...
`queue_tasks` and `queue_attempts` tables therefore live beside observed accounts,
...
a username or profile URL and saves its identity without scheduling archival.
`POST /api/accounts` confirms
...
Pushing a version tag such as `v0.1.2-rc3` publishes two multi-platform images to
...
```sh
curl -fsSL https://raw.githubusercontent.com/INP146/ArchiveX/v0.1.2-rc3/docker-compose.ghcr.yml -o ./docker-compose.yml
```
...
scheduler, and `tools` all use the backend image. It pins version `0.1.2-rc3`
...
**Upgrading from rc2 to rc3 requires an offline schema v2 to v3 migration before
starting the new application.** Reposts now reference canonical content and
media, rather than keeping a separate media copy for each event. Stop all
ArchiveX writers and drain the queues first. See
[the rc3 migration instructions](.agents/RELEASE_V0.1.2_RC3.md) for the normal
full-backup procedure and the compact procedure for disks that cannot hold a
second copy of media. Do not simply start rc3 against an unmigrated v2 database.
